...
def save_expense(cursor, db):
    ...
    with st.form(key='expense_submit_form', clear_on_submit=False, border=True):
        ...
        document_upload = st.file_uploader('Upload Document', 
                                           type=['txt','pdf', 'jpg', 'png', 'jpeg'], 
                                           accept_multiple_files=True, key='document_upload')
        
        if st.form_submit_button(label='Submit'):
            if not(expense_date and category and amount and document_upload):
                st.error('Please fill all the * fields and upload at least one document')
            else:
    ...
    columns = ['expense_date', 'category', 'amount', 'notes']   
